...
const faqData = [
  ...
  { q:{th:"เว็บไซต์สำหรับผู้ใหญ่ก็ตรวจสอบด้วยไหม?",en:"Do you scan adult content sites?"},
    a:{th:"ตรวจสอบด้วยครับ Lisa สแกนเว็บผู้ใหญ่ บอร์ดภาพ และแพลตฟอร์มสมาชิกต่างๆ ด้วย แล้วดำเนินการลบทั้งผ่านช่องทางอัตโนมัติและช่องทางกฎหมาย",
       en:"Yes. Lisa scans adult websites, image boards, and subscription platforms, acting through automated and legal channels."} },
  { q:{th:"เอเจนซี่จัดการนักแสดงหลายคนพร้อมกันได้ไหม?",en:"Can agencies manage multiple talent?"},
    a:{th:"ได้เลยครับ แผนองค์กรของเราออกแบบมาเพื่อเอเจนซี่โดยเฉพาะ มีแดชบอร์ดรวมดูได้ทีเดียวทั้งสังกัด ทำรายงานแบบ White-label ได้ พร้อม API และผู้จัดการบัญชีเฉพาะคอยดูแล",
       en:"Absolutely. Our Enterprise plan includes a unified dashboard, individual alert controls, white-label reporting, API access, and a dedicated account manager."} },
];
...
export function Faq() {
  ...
  return (
    <section id="faq" className="bg-[#050505] py-28">
      <div className="w-full max-w-3xl mx-auto px-6 sm:px-8">
        ...
        <div className="rounded-2xl border border-white/8 overflow-hidden bg-black
                        divide-y divide-white/5">
          {faqData.map((item, i) => (
            <div key={i}>
              <button onClick={() => setOpen(open===i ? null : i)}
                className="w-full flex items-start justify-between gap-5 px-7 py-5
                ...
                <motion.span animate={{ rotate: open===i ? 45 : 0 }} transition={{ duration:0.18 }}
                  className="text-white/25 text-2xl leading-none shrink-0 mt-0.5">
                  +
                </motion.span>
              </button>
              <AnimatePresence initial={false}>
                {open===i && (
                  <motion.div key="c"
                    ...
                    <div className="px-7 pb-6 text-white/35 text-base leading-relaxed">
                      {item.a[locale]}
                    </div>
                  </motion.div>
                )}
